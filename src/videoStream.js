...
class VideoStream extends EventEmitter {
  ...
  stream2Socket() {
	...
	if(socketServer.connectionCount == 0){
		console.log("There are no clients connected, stopping ffmpeg process.\nKilled FFMpeg process with PID => "+globalPid);
		spawn('kill', [globalPid]);
	}
  ...
  start() {
	...
	globalPid = this.mpeg1Muxer.pid;
	console.log("Spawned FFMpeg process with PID => "+this.mpeg1Muxer.pid);
  ...
  stop() {
      this.server.close();
	  console.log('Stopping FFmpeg process with PID => '+this.mpeg1Muxer.pid);
	  spawn('kill', [this.mpeg1Muxer.pid]);
